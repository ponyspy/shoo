...
module.exports = function(Model) {
	...
	module.index = function(req, res, next) {
		Publication.find().sort('-date').limit(10).exec(function(err, publications) {
			...
			Publication.countDocuments().exec(function(err, count) {
				if (err) return next(err);
	...
	module.get_list = function(req, res, next) {
		...
		Query.clone().countDocuments(function(err, count) {
			...
			Query.clone().find().sort('-date').skip(+post.context.skip).limit(+post.context.limit).exec(function(err, publications) {
				if (err) return next(err);
